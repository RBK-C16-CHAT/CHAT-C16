...
function Card({ user }) {
   ...
   return (
      <div class="flex-1 p:2 sm:p-6 justify-between flex flex-col h-96 border-4 border-red-500 mb-8 overflow-y-auto rounded w-3/5 mx-72">
         <div class="flex sm:items-center justify-between py-3 border-b-2 border-gray-200">
            <div class="flex items-center space-x-4">
               ...
               <div class="flex flex-col leading-tight">
                  ...
                  <span className="text-lg text-gray-600">member</span>
               </div>
            </div>
         </div>
                  ...
                  if (e.user !== user) {
                     return (
                        <div className="flex items-end">
                           <div >
                              <p classname="text-sm">{e.user}</p>
                           </div>
                           <div className="flex flex-col space-y-2 text-xs max-w-xs mx-2 order-2 items-start">

                              <div k={key}><span className="px-4 py-2 rounded-lg inline-block rounded-bl-none bg-gray-300 text-gray-600" >{e.message}</span></div>
                     ...
                     return (
                        <div className="flex items-end justify-end">
                           ...
                           <div className="flex flex-col space-y-2 text-xs max-w-xs mx-2 order-1 items-end">
                              <div k={key}><span className="px-4 py-2 rounded-lg inline-block rounded-br-none bg-green-500 text-white ">{e.message} </span></div>
                           </div>
            ...
            <div className="relative flex">
               <form className="w-full" onChange={handleChange}>
                  ...
                  <div className="absolute right-0 items-center inset-y-0 hidden sm:flex">
                     <button onClick={handleSubmit} type="button" className="inline-flex items-center justify-center rounded-full h-12 w-12  transition duration-500 ease-in-out text-white bg-red-400 hover:bg-yellow-400 focus:outline-none">
